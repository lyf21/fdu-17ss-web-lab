...
<body>
    ...
    <main class="container">
        ...
        <div class="btn-group countryButtons" role="group" aria-label="...">
              ...
              //<a href="list.php?country=Canada" role="button" class="btn btn-default"> Canada</a> 
				sort($countries);	//对数组进行升序排序，并重新赋值
				$arr1 = array_values($countries);
				for($i = 0; $i < count($arr1); $i ++){
					echo '<a href="list.php?country='. $arr1[$i]. '" role="button" class="btn btn-default"> '. $arr1[$i]. '</a>';
				}
   ...
   //              </a>
			// </li>
			$arr2 = array_values($images);
			for($i = 0; $i < sizeof($arr2, 0); $i ++){
				echo '<li><a href="detail.php?id='. $arr2[$i]["id"]. '" class="img-responsive">
				<img src="images/square/'. $arr2[$i]["path"]. '" alt="'. $arr2[$i]["title"]. '">
				<div class="caption"><div class="blur"></div><div class="caption-text"><h1>'
				. $arr2[$i]["title"]. '</h1></div></div></a></li>';
			}
          ?>
